...
BACKEND_SCRIPT = "core_engine/api_backend.py"

def start_pipeline():
    print("🚀 --- DÉMARRAGE DU PIPELINE SMARTQA ---")

    # 1. Lancement du Backend IA
    print("📡 [1/2] Initialisation du serveur IA (FastAPI)...")
    try:
        backend_proc = subprocess.Popen([sys.executable, BACKEND_SCRIPT])
        print(f"✅ Backend lancé avec le PID: {backend_proc.pid}")
    except Exception as e:
        print(f"❌ Erreur critique lors du lancement du backend : {e}")
        return

    # Pause pour laisser RoBERTa charger ses poids en mémoire
    print("⏳ Attente du chargement du modèle (8s)...")
    time.sleep(20) 

    # 2. Lancement des tests Cucumber
    print(f"🧪 [2/2] Exécution des tests Cucumber dans : {JAVA_PROJECT_PATH}")
    
    try:
        # Exécution synchrone de Maven
        ...
        # 3. Nettoyage (Optionnel : garde le backend ouvert pour voir le popup)
        print("\n🏁 Pipeline terminé.")
        print("📌 Note : Le backend reste actif. Appuyez sur CTRL+C pour l'arrêter.")
        
        # Si tu veux fermer le backend automatiquement, décommente la ligne suivante :
